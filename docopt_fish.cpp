...
struct argv_separation_state_t {
    ...
    argv_separation_state_t(const string_list_t &argv_, const option_list_t &options_, parse_flags_t flags_) : argv(argv_), options(options_), flags(flags_)
    {}
...
static bool parse_long_or_unseparated_short(argv_separation_state_t *st, option_t::name_type_t type, resolved_option_list_t *out_result, error_list_t *out_errors, rstring_t *out_suggestion)
{
    ...
    // Parse the argument into an 'option'. Note that this option does not appear in the options list
    // because its range reflects the string in the argument.
    // TODO: Need to distinguish between equivalent ways of specifying parameters (--foo=bar and --foo bar)
    ...
    const option_list_t matches = st->filter_options([&](const option_t &opt) {
        ...
        if (type == option_t::single_short) {
            does_match = does_match && opt.has_value() && (relaxed_separators || opt.separator == option_t::sep_none);
        }
        return does_match;
    });
    ...
    if (match_count < 1) {
        ...
        if (match.has_value()) {
            if (arg_as_option.has_value()) {
                ...
                // The range is the value portion; the index is the same as our argument.
                // TODO: suggestions
...
static void separate_argv_into_options_and_positionals(const string_list_t &argv, const option_list_t &options, parse_flags_t flags, positional_argument_list_t *out_positionals, resolved_option_list_t *out_resolved_options, error_list_t *out_errors, rstring_t *out_suggestion = nullptr) {
    ...
    while (st.idx < argv.size()) {
        if (st.saw_double_dash) {
            ...
            // Leading long option
            if (parse_long_or_unseparated_short(&st, option_t::double_long, out_resolved_options, out_errors, out_suggestion)) {
            ...
            if (parse_long_or_unseparated_short(&st, option_t::single_long, out_resolved_options, &local_long_errors, out_suggestion)) {
                // parse_long succeeded
            } else if (parse_long_or_unseparated_short(&st, option_t::single_short, out_resolved_options, &local_short_errors, out_suggestion)) {
...
public:
    ...
    // (two states may have identical progress values but be different) but it does capture
    // the relationship between a state and another state derived from it, i.e. if the
...
public:
    ...
    match_context_t(parse_flags_t f, const option_list_t &shortcut_opts, const positional_argument_list_t &p, const resolved_option_list_t &r, const string_list_t &av) : flags(f), shortcut_options(shortcut_opts), positionals(p), resolved_options(r), argv(av)
    {}
...
static bool match_options(const option_list_t &options_in_doc, match_state_t *state, const match_context_t *ctx);
...
// Ellipsis helper
// Given a node that has ellipsis, the set of resulting states, and an initial prior state count,
// match the node repeatedly until we stop making progress
template<typename Node>
static void repeat_matching(const Node &node, size_t init_prior_state_count, match_context_t *ctx, match_state_list_t *resulting_states) {
...
public:
    ...
    /* Matches argv */
    void match_argv(const string_list_t &argv,
                    parse_flags_t flags,
                    const positional_argument_list_t &positionals,
                    const resolved_option_list_t &resolved_options,
    ...
    void best_assignment_for_argv(const string_list_t &argv, parse_flags_t flags, error_list_t *out_errors, index_list_t *out_unused_arguments, option_rmap_t *out_option_map) const
    {
...
argument_parser_t::parse_arguments(const string_list_t &argv,
                                                parse_flags_t flags,
