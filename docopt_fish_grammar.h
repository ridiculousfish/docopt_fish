...
struct options_shortcut_t {
    // The options shortcut does not need to remember its token, since we never
    // use it
    // It's always assuemd to be present
    bool present = false;

    std::string name() const {
        return "options_shortcut";
    }
    template <typename T>
...
usage_t parse_one_usage(const rstring_t &src, const option_list_t &shortcut_options,
...
struct node_visitor_t {
    ...
    /* Function called from overrides of visit_children. We invoke an override of
     * accept(), and then
     * recurse to children. */
    template <typename NODE_TYPE>
    ...
    /* Visit is called from node visit_children implementations. A token has no
     * children. */
